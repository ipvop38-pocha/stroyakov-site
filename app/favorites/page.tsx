...
export default function FavoritesPage() {
  ...
  function add(product:CatalogProduct){if(product.price===null||product.stock===null||product.stock<=0)return;const current=readCart();const id=`product-${product.id}`;writeCart([...current.filter((item)=>item.id!==id),{id,title:product.name,price:product.price,quantity:1,image:product.image,detail:`${product.brand} · ${product.unit}`}]);setAdded(v=>[...v,product.id]);}
