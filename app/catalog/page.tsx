...
import { CatalogCategoryArt } from "../components/catalog-category-art";
...
import { brandFilterValue, availableFacets, facetDefinitions, matchesFacets, productFacets, readFacetSelection, type FacetSelection, type TileContext } from "../lib/catalog-facets";
...
export default function CatalogPage() {
  ...
  const [tileContext, setTileContext] = useState<TileContext>("indoor");
  ...
  useEffect(() => {
    ...
    const requestedContext = url.searchParams.get("tileContext");
    const initialContext: TileContext = requestedContext === "outdoorWall" || requestedContext === "outdoorFloor" ? requestedContext : "indoor";
    setTileContext(initialContext);
    setSelectedFacets(readFacetSelection(url.searchParams, availableFacets(initialProducts, facetDefinitions(initialCategory, initialSubgroup, initialContext), {})));
    setBrands(url.searchParams.getAll("brand").filter(brand => initialProducts.some(product => brandFilterValue(product) === brand)));
  ...
  const allBrands = useMemo(() => {
    ...
    return [...new Set(products.map(brandFilterValue))].sort((left, right) => {
    const count = (brand: string) => products.filter(product => brandFilterValue(product) === brand).length;
  ...
  const candidates = useMemo(() => scopedProducts.filter(product =>
    ...
    (!brands.length || brands.includes(brandFilterValue(product))) && matchesProductSearch(product, query)
  ), [scopedProducts, availability, brands, query]);
  const facets = useMemo(() => availableFacets(scopedProducts, facetDefinitions(category, subgroup, tileContext), selectedFacets, candidates), [scopedProducts, category, subgroup, tileContext, selectedFacets, candidates]);
  ...
  function updateUrl(nextCategory = category, nextSubgroup = subgroup, nextAvailability = availability, nextQuery = query, nextFacets = selectedFacets, nextBrands = brands, nextTileContext = tileContext) {
    ...
    if (nextSubgroup === "Плиточные клеи" && nextTileContext !== "indoor") url.searchParams.set("tileContext", nextTileContext); else url.searchParams.delete("tileContext");
  ...
  function chooseTileContext(value: TileContext) {
    const next = Object.fromEntries(Object.entries(selectedFacets).filter(([id]) => !id.startsWith("tileSize_")));
    setTileContext(value); setSelectedFacets(next); updateUrl(category, subgroup, availability, query, next, brands, value);
  }
  ...
  return <SiteChrome active="catalog"><div className="inner-canvas catalog-page">
    ...
    <div className="catalog-layout"><aside ref={filtersRef} role={filtersOpen ? "dialog" : undefined} aria-modal={filtersOpen || undefined} aria-label="Фильтры каталога" className={`catalog-filters ${filtersOpen ? "is-open" : ""}`}><button aria-label="Закрыть фильтры" className="filter-close" onClick={() => setFiltersOpen(false)} type="button"><X/></button><h2>Фильтры</h2>
      ...
      {facets.length > 0 && <div className="property-filters" aria-label={`Свойства: ${subgroup}`}>
        ...
        {subgroup === "Плиточные клеи" && <label className="tile-context">Где укладываем керамогранит<select value={tileContext} onChange={event => chooseTileContext(event.target.value as TileContext)}><option value="indoor">Внутри помещения</option><option value="outdoorWall">Снаружи — стены</option><option value="outdoorFloor">Снаружи — пол</option></select></label>}
        {facets.map(facet => <fieldset className="property-filter" key={facet.id}><legend>{facet.label}</legend>{facet.help && <details className="property-filter-help"><summary>Как выбрать: {facet.label.toLowerCase()}</summary><p>{facet.help}</p></details>}{facet.options.map(option => {
