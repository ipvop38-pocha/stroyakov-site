...
export default function CatalogPage() {
  ...
  const filtered=useMemo(()=>catalogProducts.filter(p=>(category==="Все товары"||p.category===category||(category==="Гипсокартон"&&p.category==="Листовые материалы"))&&(!inStock||(p.stock!==null&&p.stock>0))&&(!brands.length||brands.includes(p.brand))&&matchesProductSearch(p,query)).sort((a,b)=>sort==="popular"?b.popularity-a.popularity:a.price===null?b.price===null?0:1:b.price===null?-1:sort==="price-asc"?a.price-b.price:b.price-a.price),[brands,category,inStock,query,sort]);
  useEffect(()=>setVisibleCount(24),[brands,category,inStock,query,sort]);
  function chooseCategory(name:string){setCategory(name);setFiltersOpen(false);const slug=categorySlugMap[name];const url=new URL(window.location.href);if(slug)url.searchParams.set("category",slug);else url.searchParams.delete("category");if(query)url.searchParams.set("q",query);else url.searchParams.delete("q");url.hash="products";history.replaceState(null,"",url);window.setTimeout(()=>document.getElementById("products")?.scrollIntoView({behavior:"smooth",block:"start"}),0);}
  ...
  function addProduct(product:CatalogProduct){if(product.price===null)return;const current=readCart();const id=`product-${product.id}`;const existing=current.find(item=>item.id===id);writeCart(existing?current.map(item=>item.id===id?{...item,quantity:item.quantity+1}:item):[...current,{id,title:product.name,price:product.price,quantity:1,image:product.image,detail:`${product.brand} · ${product.unit}`}]);setAdded(value=>[...new Set([...value,product.id])]);}
  ...
  return <SiteChrome active="catalog"><div className="inner-canvas catalog-page">
    ...
    <div className="catalog-layout"><aside className={`catalog-filters ${filtersOpen?"is-open":""}`}><button aria-label="Закрыть фильтры" className="filter-close" onClick={()=>setFiltersOpen(false)} type="button"><X/></button><h2>Фильтры</h2><div className="filter-group"><b>Категория</b>{["Все товары",...new Set(catalogProducts.map(p=>p.category))].map(item=><button className={category===item?"is-active":""} key={item} onClick={()=>chooseCategory(item)} type="button"><span/>{item}<em>{item==="Все товары"?catalogProducts.length:catalogProducts.filter(p=>p.category===item).length}</em></button>)}</div><div className="filter-group"><b>Наличие</b><button className={inStock?"is-active":""} onClick={()=>setInStock(!inStock)} type="button"><span/>В наличии</button></div><div className="filter-group"><b>Бренды</b>{[...new Set(catalogProducts.map(p=>p.brand).filter(Boolean))].map(item=><button className={brands.includes(item)?"is-active":""} key={item} onClick={()=>setBrands(current=>current.includes(item)?current.filter(brand=>brand!==item):[...current,item])} type="button"><span/>{item}</button>)}</div><div className="filter-note"><b>Остатки из МоегоСклада</b><p>Наличие и итоговую стоимость подтвердим перед отгрузкой.</p></div></aside><section className="catalog-results"><div className="catalog-result-meta"><span>{filtered.length} позиции</span><small>Цены указаны за единицу товара</small></div>{filtered.length?<><div className="catalog-product-grid">{filtered.slice(0,visibleCount).map(product=><CatalogProductCard added={added.includes(product.id)} favorite={favorites.includes(product.id)} key={product.id} onAdd={()=>addProduct(product)} onFavorite={()=>toggleFavorite(product.id)} product={product}/>)}</div>{visibleCount<filtered.length&&<button className="catalog-load-more" onClick={()=>setVisibleCount(count=>count+24)} type="button">Показать ещё 24 товара</button>}</>:<div className="catalog-empty"><h2>Товары не найдены</h2><p>Попробуйте другой запрос или сбросьте фильтры. Если нужного товара пока нет в каталоге, поможем подобрать его по заявке.</p><button onClick={()=>{setCategory("Все товары");setQuery("");setBrands([]);setInStock(false);}} type="button">Показать доступные товары</button></div>}</section></div>
  </div></SiteChrome>;
}
